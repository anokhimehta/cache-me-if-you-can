    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if (
                hasattr(self.config, "t2v_keep_ratios")
                and self.config.t2v_keep_ratios is not None
                and layer_idx in self.config.t2v_keep_ratios
                and visual_pos_masks is not None
            ):
                text_mask = ~visual_pos_masks
                vision_mask = visual_pos_masks

                text_positions = torch.nonzero(text_mask[0], as_tuple=False).squeeze(-1)
                vision_positions = torch.nonzero(vision_mask[0], as_tuple=False).squeeze(-1)

                if vision_positions.numel() == 0:
                    continue

                t2v_attn = attn_weights[:, :, text_positions, :][:, :, :, vision_positions]
                vision_scores = t2v_attn.mean(dim=(1, 2))[0]

                keep_ratio = self.config.t2v_keep_ratios[layer_idx]
                num_vision = vision_scores.shape[0]

                K = max(1, min(int(num_vision * keep_ratio), num_vision))
                
                keep_local_idx = torch.topk(vision_scores, K).indices
                keep_positions = vision_positions[keep_local_idx]
                
                #keep_positions = keep_positions.clamp(0, hidden_states.shape[1] - 1)
                
                # text_idx = torch.where(text_mask[0])[0]
                # vision_idx = torch.where(vision_mask[0])[0]
                new_positions = torch.cat([text_positions, keep_positions])
                new_positions = torch.unique(new_positions)   # 🔥 avoid duplicates
                new_positions, _ = torch.sort(new_positions)

                #new_positions = new_positions[new_positions < hidden_states.shape[1]]
                
                hidden_states = hidden_states[:, new_positions, :]
                visual_pos_masks = visual_pos_masks[:, new_positions]

                if position_ids is not None:
                    position_ids = position_ids[:, :, new_positions]

                if text_position_ids is not None:
                    text_position_ids = text_position_ids[:, new_positions]
                ...
                # if num_vision == 0 or text_idx.numel() == 0:
                #     continue

                # t2v_attn = attn_weights.index_select(2, text_idx)
                # t2v_attn = t2v_attn.index_select(3, vision_idx)
                


                # # vision_positions = torch.where(vision_mask[0])[0]
                # # text_positions = torch.where(text_mask[0])[0]

                
                # #     if num_vision == 0 or text_idx.numel() == 0:
                # #         continue
                
                # # t2v_attn = attn_weights[:, :, text_mask[0], :][:, :, :, vision_mask[0]]
                
                # # vision_scores = t2v_attn.mean(dim=(1, 2))[0].squeeze(0)
                # # vision_scores = vision_scores[:num_vision]
                
                # keep_ratio = self.config.t2v_keep_ratios[layer_idx]
                # K = int(num_vision * keep_ratio)
                # K = max(1,min(K,num_vision))
               
                # keep_local_idx = torch.topk(vision_scores, K).indices
                # keep_positions = vision_idx.index_select(0, keep_local_idx)
                # text_positions = text_idx
                # new_positions = torch.cat([text_positions, keep_positions])
                # new_positions, _ = torch.sort(new_positions)


                # if new_positions.max() >= hidden_states.shape[1]:
                #     print("Invalid T2V index, skipping pruning")
                #     continue
                ...
                # #     attention_mask = attention_mask[:, new_positions]
                # if position_ids is not None:
                #     position_ids = position_ids[:, :, new_positions]
                ...
                # print("visual_mask:", visual_pos_masks.shape)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            if hasattr(self.config, "visionzip_keep_ratio") and self.config.visionzip_keep_ratio is not None:
            
                keep_ratio = self.config.visionzip_keep_ratio
            
                N = image_embeds.shape[0]
                K = max(1, int(N * keep_ratio))
            
                # importance scoring
                scores = torch.norm(image_embeds, dim=1)
            
                keep_idx = torch.topk(scores, K).indices
                keep_idx, _ = torch.sort(keep_idx)  # preserve order
            
                # drop tokens
                image_embeds = image_embeds[keep_idx]
            
                # update deepstack
                if deepstack_image_embeds is not None:
                    new_deepstack = []
                    for ds in deepstack_image_embeds:
                        new_deepstack.append(ds[keep_idx])
                    deepstack_image_embeds = new_deepstack
                    
                image_mask_flat = image_mask[..., 0].reshape(-1)
                
                image_positions = torch.where(image_mask_flat)[0]
                kept_positions = image_positions[keep_idx]

                non_image_positions = torch.where(~image_mask_flat)[0]

                new_positions = torch.cat([non_image_positions, kept_positions])
                new_positions, _ = torch.sort(new_positions)

                inputs_embeds = inputs_embeds[:, new_positions, :]
                ...
                if input_ids is not None:
                    input_ids = input_ids[:, new_positions]

                if mm_token_type_ids is not None:
                    mm_token_type_ids = mm_token_type_ids[:, new_positions]
                ...
                new_image_mask_flat = image_mask_flat[new_positions]
                new_image_mask = new_image_mask_flat.view(inputs_embeds.shape[0], inputs_embeds.shape[1])

                image_mask = new_image_mask.unsqueeze(-1).expand_as(inputs_embeds)


            


                # new_mask_flat = torch.zeros_like(image_mask_flat)
                # new_mask_flat[kept_positions] = True
                
                # new_mask = new_mask_flat.view(image_mask.shape[0], image_mask.shape[1])
                # #new_mask[kept_positions] = True
                # image_mask = new_mask.unsqueeze(-1).expand_as(inputs_embeds)
        ...
        if self.config.visionzip_keep_ratio is not None:
            position_ids = None
        else:
