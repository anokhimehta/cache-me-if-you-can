...
import sys
sys.path.append("/scratch/al9581/project_work/complete")
import torch
...
def generate_answer(
    ...
    t2v: bool = False,
    t2v_keep_ratios: Optional[Dict[int, float]] = None,
...
############################################
    ...
    image_tokens_before = inputs["pixel_values"].shape[0] if "pixel_values" in inputs else None
    if image_tokens_before is not None:
        if visionzip_keep_ratio is not None:
            image_tokens_after = int(image_tokens_before * visionzip_keep_ratio)
        else:
            image_tokens_after = image_tokens_before
    else:
    ...
    if visionzip_keep_ratio is not None:
        model.config.visionzip_keep_ratio = visionzip_keep_ratio
    else:
        model.config.visionzip_keep_ratio = None

    if t2v:
        if t2v_keep_ratios is not None:
            t2v_keep_ratios = {14: 0.5, 21: 0.5}
        
        model.config.text_config.t2v_keep_ratios = t2v_keep_ratios
        model.model.language_model.config.t2v_keep_ratios = t2v_keep_ratios
    else:
        model.config.text_config.t2v_keep_ratios = None
        model.model.language_model.config.t2v_keep_ratios = None
    
    # model.config.t2v_keep_ratios = t2v_keep_ratios
    start = time.time()
    with torch.no_grad():
